...
def explore_this_lr(netD, netG, lr, data, label, device = "cuda", n_dis = 1, nz=128, exploration_epoch = 5):


    # get our model and define the optimizer as well as the loss criterion
    lr = lr[0]
    optD = optim.Adam(netD.parameters(), lr, betas=(0.5, 0.999))
    optG = optim.Adam(netG.parameters(), lr, betas=(0.5, 0.999))

    mcr_gan_loss = MCRGANloss(gam1=cfg.LOSS.GAM1, gam2=cfg.LOSS.GAM2, gam3=cfg.LOSS.GAM3, eps=cfg.LOSS.EPS, numclasses=2, mode=0, rho=cfg.LOSS.RHO)


    for epoch in range(exploration_epoch):
        ...
        for i in range(n_dis):
            ...
            X_bar = netG(torch.reshape(Z.detach(), (len(Z), nz)))
            ...
            errD, errD_EC = mcr_gan_loss(Z, Z_bar, label, i, n_dis)
        ...
        errG, errG_EC = mcr_gan_loss(Z, Z_bar, label, n_dis - 1, n_dis)

        errG = (-1) * errG
        errG.backward()
        optG.step()

    #return np.asscalar((2*(errD -errG) - errG_EC[1]).cpu().detach().numpy())
    return np.asscalar((-2*errG- errG_EC[1]).cpu().detach().numpy())


def search_lr(netD, netG, data, label, current_lr, search_iter = 10):

    n_iter = search_iter  # number of iterations
    ...
    bo = BayesianOptimizer(lambda x: explore_this_lr(copy.deepcopy(netD), copy.deepcopy(netG), x, data, label),
                           gp,
                           mode="linear",
                           bound=[lr_lb, lr_hb])
    ...
    found_lr = gp_minimize(lambda x: explore_this_lr(copy.deepcopy(netD), copy.deepcopy(netG), x, data, label),                  # the function to minimize
    ...
    def train(self):
        ...
        try:
            ...
            while global_step < self.num_steps:
                ...
                if global_step % adjust_lr == 0:
                    lr_old = self.optD.param_groups[0]['lr']
                    lr = search_lr(copy.deepcopy(self.netD), copy.deepcopy(self.netG), real_cpu, real_label, lr_old)
                    print("Learning rate has been updated from:", lr_old, "to", lr)
                ...
                for i in range(self.n_dis):
                    ...
                    # Generate batch of latent vectors (Z->X')
                    X_bar = self.netG(torch.reshape(Z.detach(), (len(Z), nz)))
                ...
                #log_data = self.scheduler.step(log_data=log_data, global_step=global_step)
